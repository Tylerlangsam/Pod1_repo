...
activity = []                #define variable activity
for day in days:             #define variable day
    x = input(f"What is your favorite thing to do on {day}? ")  #set x as variable to store input
    activity.append(x)       #populate variable activity
print(activity)
...
for i in range(len(days)):   #set variable i to loop  through list days used len to make sure the variables from bolth lists match
    print( f'"My favorite thing to do on {days[i]} is {activity[i]}"') #print a new list with variables from both lists
...
temp = input(('What is the temperature today?')) #define "temp" with user input
if temp < '50':
...
num = int(input('How many times would you like to be wished happy birthday? '))
for i in range(num):
