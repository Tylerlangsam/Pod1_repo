...
class SubwayStation(Station):
    def__init__(self, station_name, location, lines):
        super().__init__(station_name, location)
        self.lines = lines 
    def show_info(self):
        print(f'Lines are {self.lines}, Station Name is {self.station_name},  Location is {self.location}")')   
...
lines: ['1', '2', '3', 'L']
'''
Station_1 = SubwayStation("14th Street","14th St & 7th Ave",['1','2','D','F'])
Station_1.show_info()
...
class BusStation(Station):
    def __init__(self, station_name, location, routes, open=True, hours="open"):
        super().__init__(station_name, location)
        self.routes = routes
        self.open = open
        self.hours = hours
    ...
    def show_info(self):
        if self.open == True:
            ...
            self.hours = "closed"
        print(f"Bus station name is {self.station_name}, located at {self.location}. Routes are {self.routes}, and is currently {self.hours}")

Station_2 = BusStation("Penn Station","34th & 8th","Hudson River, New Jersey")

Station_2.show_info()

Station_2.close_station()
...
'''
nyc_megabus = BusStation(station_name, location, lines)
