...
def validate_user_input1():
    usr_inpt = input(f"Please enter a number ")
    while usr_inpt.isnumeric() == False:
        print('You did not enter a valid number, please try again')
        usr_inpt = input(f"Please enter a number")

    return usr_inpt
  
# validate_user_input1()

def validate_user_input2():
    usr_inpt = input(f"Please enter a number ")
    try:
        chk_usr_inpt = int(usr_inpt)
...
def print_tenth_item(top_ten):    
    try:
        ...
        print('There are not ten items in this list')
        print(e) 
